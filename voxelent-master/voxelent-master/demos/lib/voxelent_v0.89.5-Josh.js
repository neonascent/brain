...
vxlModelManager.prototype.load = function(uri, scene) {
	...
	if (dtype=="zip") {
		var loader = new ZipLoader(uri);
		
		// hardwire for testing working locations 
		//var zipcontents = loader.load(uri+'://'+modelname+'.json');
		var zipcontents = loader.load('http://localhost/brain/voxelent-master/voxelent-master/demos/brain.zip://brain.json');
		alert("wait");
		manager = jQuery.parseJSON(zipcontents);
		json.uri = filename;
		json.path = vxl.util.getPath(uri);
		var scene = new jqXHR();
		manager.add(json,modelname,scene);
